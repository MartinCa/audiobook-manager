<template>
    ...
    <v-row>
        <v-col>
            <v-expansion-panels v-model="activePanel">
                ...
                <v-expansion-panel v-for="(mappings, key) in groupedSeriesMappings"
                                   :key="mappings.mapped_series">
                    <v-expansion-panel-title>
                        <v-row>
                            <v-col>
                                {{ mappings.mapped_series }}
                            </v-col>
                        </v-row>
                    </v-expansion-panel-title>
                    <v-expansion-panel-text>
                        <SeriesMappingInput v-for="(mapping, i) in mappings.mappings"
                                            :key="mapping.id"
                                            :mapping="mapping"
    ...
    <v-dialog v-model="showCreateDialog"
        ...
        <v-card :width="dialogWidth">
            ...
            <v-card-text>
                <SeriesMappingInput @mapping-deleted="showCreateDialog = false"
...
import { GroupMapping, SeriesMapping, SeriesMappingBase } from '../../types/SeriesMapping';
...
function OrderByArray<T, K extends keyof T>(values: T[], orderType: K) {
    return values.sort((a, b) => (a[orderType] > b[orderType]) ? 1 : (a[orderType] < b[orderType]) ? -1 : 0)
}

const groupedSeriesMappings = computed((): GroupMapping[] => {
    const mappingObject = seriesMappings.value.reduce<{ [m: string]: SeriesMapping[] }>((r, v, i, a, k = v.mapped_series) => ((r[k] || (r[k] = [])).push(v), r), {})

    return OrderByArray(Object.entries(mappingObject).map((v) => ({ mapped_series: v[0], mappings: v[1] })), "mapped_series");
});

const mappingDeleted = (mapping: SeriesMapping | undefined) => {
    if (mapping) {
        seriesMappings.value = seriesMappings.value.filter(sm => sm.id != mapping.id);
    }
}
