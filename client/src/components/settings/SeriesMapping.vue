...
const groupedSeriesMappings = computed((): GroupMapping[] => {
  const mappingObject = seriesMappings.value.reduce<{ [m: string]: SeriesMapping[] }>((r, v, i, a, k = v.mappedSeries) => ((r[k] || (r[k] = [])).push(v), r), {})

  return OrderByArray(Object.entries(mappingObject).map((v) => ({ mappedSeries: v[0], mappings: v[1] })), "mappedSeries");
});
