  ...
  <v-card :width="dialogWidth">
    ...
    <v-card-text>
      ...
      <v-row>
        <v-col>
          <v-list lines="two">
            <v-list-item v-for="dirContent in directoryContents"
...
onMounted(async () => {
  if (props.bookDetails.fileInfo) {
    directoryContents.value = await FilesService.getDirectoryContents(props.bookDetails.fileInfo.fullPath);
  } else {
    closeDialog();
  }
});
...
const formatFileSize = (fileSize: number) => `${(fileSize / 1000000).toFixed(2)} MB`;
