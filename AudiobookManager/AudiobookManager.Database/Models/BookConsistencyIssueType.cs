...
public enum BookConsistencyIssueType
{
    MissingMediaFile = 0,
    WrongFilePath = 1,
    MissingDescTxt = 2,
    ...
    /// non-compliant person value, with <see cref="BookConsistencyIssue.AudiobookId"/> naming a
    /// representative book the person appears on, and <see cref="BookConsistencyIssue.ExpectedValue"/>/
    /// <see cref="BookConsistencyIssue.ActualValue"/> carrying the canonical vs stored spelling.
    ...
    /// <see cref="BookConsistencyIssueType.UnreadableFile"/>, where resolving is also just looking
    /// again). <see cref="BookConsistencyIssue.ActualValue"/> carries the error message, and
    ...
    /// series detail renders, so the two can never disagree. <see cref="BookConsistencyIssue.ExpectedValue"/>
    /// carries the roster's position (which resolving writes into the m4b and database through the
    /// binding-invariant no-DB-only-update pipeline) and <see cref="BookConsistencyIssue.ActualValue"/>
