...
{
    public static QueuedOrganizeTask ToDomain(this Database.Models.QueuedOrganizeTask dbEntity) => new QueuedOrganizeTask(dbEntity.OriginalFileLocation, JsonSerializer.Deserialize<Audiobook>(dbEntity.JsonAudiobook) ?? throw new InvalidOperationException($"Failed to deserialize audiobook JSON for queued task at '{dbEntity.OriginalFileLocation}'"), dbEntity.QueuedTime);
