...
{
    private static GoodreadsScraper CreateScraper()
    {
    ...
    public async Task ParseNewBookJson()
    {
        ...
        // Publisher
        Assert.AreEqual("Modtryk", parseResult.Publisher);

        // Language
        Assert.AreEqual("Danish", parseResult.Language);

        // ISBN
        Assert.AreEqual("9788770530149", parseResult.Isbn);

        // ASIN
        Assert.AreEqual("8770530149", parseResult.Asin);

        // Description should be sanitized (no HTML tags)
        Assert.IsFalse(parseResult.Description.Contains("<br />"));
        Assert.IsFalse(parseResult.Description.Contains("<"));
    }

    [TestMethod]
    public async Task ParseNewBookJson_TheHobbit()
    {
        var target = CreateScraper();
        var jsonText = await File.ReadAllTextAsync("Scraping/TestData/hobbit.json");
        var bookUrl = "https://www.goodreads.com/book/show/5907.The_Hobbit_or_There_and_Back_Again";

        var result = await target.ParseNewBookJson(jsonText, bookUrl);

        Assert.IsNotNull(result);

        // Title - contains comma, should not be split (no colon)
        Assert.AreEqual("The Hobbit, or There and Back Again", result.BookName);
        Assert.IsNull(result.Subtitle);

        // URL
        Assert.AreEqual(bookUrl, result.Url);

        // Authors - illustrators should be filtered out
        Assert.AreEqual(2, result.Authors.Count);
        var tolkien = result.Authors.SingleOrDefault(x => x.Name == "J.R.R. Tolkien");
        Assert.IsNotNull(tolkien);
        Assert.AreEqual("Author", tolkien.Role);
        var anderson = result.Authors.SingleOrDefault(x => x.Name == "Douglas A. Anderson");
        Assert.IsNotNull(anderson);
        Assert.AreEqual("Editor", anderson.Role);

        // Narrators
        Assert.AreEqual(0, result.Narrators.Count);

        // Year - from work publicationTime (-1018627200000 = 1937)
        ...
        // Genres - "Fiction" is filtered, so first 5 of: Fantasy, Classics, Adventure, Young Adult, Audiobook
        Assert.AreEqual(5, result.Genres.Count);
        Assert.IsTrue(result.Genres.Contains("Fantasy"));
        Assert.IsTrue(result.Genres.Contains("Classics"));
        Assert.IsFalse(result.Genres.Contains("Fiction"));

        // Rating
        Assert.IsTrue(result.Rating >= 4.0);
        Assert.IsTrue(result.NumberOfRatings >= 4000000);

        // Publisher
        Assert.AreEqual("Houghton Mifflin", result.Publisher);

        // Language
        Assert.AreEqual("English", result.Language);

        // ISBN - null for this edition
        Assert.IsNull(result.Isbn);

        // ASIN
        Assert.AreEqual("B0DN8P5BQF", result.Asin);
    }

    [TestMethod]
    public async Task ParseNewBookJson_TheShining()
    {
        var target = CreateScraper();
        var jsonText = await File.ReadAllTextAsync("Scraping/TestData/shining.json");
        var bookUrl = "https://www.goodreads.com/book/show/11588.The_Shining";
        ...
        // Title
        Assert.AreEqual("The Shining", result.BookName);
        Assert.IsNull(result.Subtitle);

        // URL
        Assert.AreEqual(bookUrl, result.Url);

        // Authors - single author, no secondary contributors
        Assert.AreEqual(1, result.Authors.Count);
        var king = result.Authors.Single();
        Assert.IsTrue(king.Name.Contains("Stephen"));
        Assert.IsTrue(king.Name.Contains("King"));
        Assert.AreEqual("Author", king.Role);

        // Narrators
        Assert.AreEqual(0, result.Narrators.Count);

        // Year - from work publicationTime (223286400000 = 1977)
        ...
        // Should contain newlines where <br /> tags were
        Assert.IsTrue(result.Description.Contains("\n"));

        // Genres - "Fiction" filtered, first 5 of: Horror, Thriller, Classics, Fantasy, Paranormal
        Assert.AreEqual(5, result.Genres.Count);
        Assert.IsTrue(result.Genres.Contains("Horror"));
        Assert.IsFalse(result.Genres.Contains("Fiction"));

        // Rating
        Assert.IsTrue(result.Rating >= 4.0);
        Assert.IsTrue(result.NumberOfRatings >= 1700000);

        // Publisher
        Assert.AreEqual("New English Library", result.Publisher);

        // Language
        Assert.AreEqual("English", result.Language);

        // ISBN
        Assert.AreEqual("9780450040184", result.Isbn);
